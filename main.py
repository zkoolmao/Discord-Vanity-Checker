...
async def checkVanity(session, mode):
    while True:
        try:
            ...
            response_data, status_code, _ = await fetch(session, f"https://discord.com/api/v9/invites/{word}")

            if status_code == 404:
                Logger.info(f"{status_code} | Vanity {word} is available/locked")
            elif status_code == 200:
                Logger.error(f"{status_code} | Vanity {word} is not available")
            elif status_code == 429:
                Logger.error(f"{status_code} | Ratelimited, sleeping for 5 seconds..")
...
async def setup():
    ...
    async with aiohttp.ClientSession() as session:
        await checkVanity(session, config["mode"])
